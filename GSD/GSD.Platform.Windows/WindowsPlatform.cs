...
using GSD.Platform.Windows.DiskLayoutUpgrades;
...
{
    public partial class WindowsPlatform : GSDPlatform
    {
        ...
        public override GSDPlatformConstants Constants { get; } = new WindowsPlatformConstants();

        public override string GSDConfigPath
        {
            get
            {
                string servicePath = GSDPlatform.Instance.GetDataRootForGSDComponent(GSDConstants.Service.ServiceName);
                string gvfsDirectory = Path.GetDirectoryName(servicePath);

                return Path.Combine(gvfsDirectory, LocalGSDConfig.FileName);
            }
        }
        ...
        public override bool TryGetGSDHooksPathAndVersion(out string hooksPath, out string hooksVersion, out string error)
        {
            error = null;
            hooksVersion = null;
            ...
            if (hooksPath == null)
            {
                error = "Could not find " + GSDPlatform.Instance.Constants.GSDHooksExecutableName;
                return false;
            }

            FileVersionInfo hooksFileVersionInfo = FileVersionInfo.GetVersionInfo(Path.Combine(hooksPath, GSDPlatform.Instance.Constants.GSDHooksExecutableName));
        ...
        {
            return File.Exists(Path.Combine(GSDPlatform.Instance.GetDataRootForGSDComponent(GSDConstants.Service.ServiceName), GSDConstants.GitStatusCache.EnableGitStatusCacheTokenFile));
        }
        ...
        public override bool TryGetDefaultLocalCacheRoot(string enlistmentRoot, out string localCacheRoot, out string localCacheRootError)
        {
            ...
            try
            {
                localCacheRoot = Path.Combine(pathRoot, GSDConstants.DefaultGSDCacheFolderName);
        ...
        public class WindowsPlatformConstants : GSDPlatformConstants
        {
            ...
            {
                get { return WindowsPlatform.DotGSDRoot; }
            }
            ...
            {
                get { return "GSD" + this.ExecutableExtension; }
            }
            ...
            {
                get { return new HashSet<string>(StringComparer.OrdinalIgnoreCase) { "GSD", "GSD.Mount", "git", "ssh-agent", "wish", "bash" }; }
            }
        }
    }
}
