...
namespace GSD.Platform.Windows
{
    public class WindowsFileBasedLock : FileBasedLock
    {
        ...
        /// <remarks>
        /// GSD keeps an exclusive write handle open to lock files that it creates with FileBasedLock.  This means that
        /// FileBasedLock still ensures exclusivity when the lock file is used only for coordination between multiple GSD processes.
        /// </remarks>
        ...
        public override bool TryAcquireLock()
        {
            ...
            catch (IOException e)
            {
                ...
                // HResultErrorSharingViolation is expected when the lock file exists andanother GSD process has acquired the lock file
