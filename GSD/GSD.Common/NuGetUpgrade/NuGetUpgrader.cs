...
namespace GSD.Common.NuGetUpgrade
{
    public class NuGetUpgrader : ProductUpgrader
    {
        ...
        public NuGetUpgrader(
            ...
            : this(
                ...
                new NuGetFeed(
                    ...
                    downloadFolder,
                    null,
                    GSDPlatform.Instance.UnderConstruction.SupportsNuGetEncryption,
                    tracer),
                credentialStore,
                GSDPlatform.Instance.CreateProductUpgraderPlatformInteractions(fileSystem, tracer))
        {
        }
        ...
        public override bool TryRunInstaller(InstallActionWrapper installActionWrapper, out string error)
        {
            ...
            using (ITracer activity = this.tracer.StartActivity(nameof(this.TryRunInstaller), EventLevel.Informational))
            {
                ...
                try
                {
                    string platformKey = GSDPlatform.Instance.Name;
        ...
        public class NuGetUpgraderConfig
        {
            ...
            protected readonly LocalGSDConfig localConfig;
            ...
            public virtual bool IsReady(out string error)
            {
                    ...
                    string.IsNullOrEmpty(this.PackageFeedName))
                {
                    error = string.Join(
                        ...
                        $"Use `gvfs config [{GSDConstants.LocalGSDConfig.UpgradeFeedUrl} | {GSDConstants.LocalGSDConfig.UpgradeFeedPackageName}] <value>` to set the config.");
                    return false;
                }
            ...
            {
                string configValue;
                if (!this.localConfig.TryGetConfig(GSDConstants.LocalGSDConfig.UpgradeFeedUrl, out configValue, out error))
                {
                ...
                if (!this.localConfig.TryGetConfig(GSDConstants.LocalGSDConfig.UpgradeFeedPackageName, out configValue, out error))
                {
                    this.tracer.RelatedError(error);
                    return false;
                }
