...
namespace GSD.Common.NuGetUpgrade
{
    public class OrgNuGetUpgrader : NuGetUpgrader
    {
            ...
            out string error)
        {
            ...
            string platform = GSDPlatform.Instance.Name;
        ...
        public class OrgNuGetUpgraderConfig : NuGetUpgraderConfig
        {
            ...
            public override bool TryLoad(out string error)
            {
                ...
                if (!this.localConfig.TryGetConfig(GSDConstants.LocalGSDConfig.OrgInfoServerUrl, out string orgInfoServerUrl, out error))
                {
                ...
                if (!this.localConfig.TryGetConfig(GSDConstants.LocalGSDConfig.UpgradeRing, out string upgradeRing, out error))
                {
                    this.tracer.RelatedError(error);
                    return false;
                }
            ...
            public override bool IsReady(out string error)
            {
                    ...
                    string.IsNullOrEmpty(this.OrgInfoServer))
                {
                    error = string.Join(
                        ...
                        "Use `gvfs config [{GSDConstants.LocalGSDConfig.UpgradeFeedUrl} | {GSDConstants.LocalGSDConfig.UpgradeFeedPackageName} | {GSDConstants.LocalGSDConfig.UpgradeRing} | {GSDConstants.LocalGSDConfig.OrgInfoServerUrl}] <value>` to set the config.");
                    return false;
                }
