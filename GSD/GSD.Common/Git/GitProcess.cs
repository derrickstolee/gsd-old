...
namespace GSD.Common.Git
{
    public class GitProcess : ICredentialStore
    {
        ...
        public GitProcess(Enlistment enlistment)
            : this(enlistment.GitBinPath, enlistment.WorkingDirectoryBackingRoot, enlistment.GSDHooksRoot)
        {
        }
        ...
        public GitProcess(string gitBinPath, string workingDirectoryRoot, string gvfsHooksRoot)
        {
            ...
            {
                this.dotGitRoot = Path.Combine(this.workingDirectoryRoot, GSDConstants.DotGit.Root);
            }
        }
        ...
        public bool TryKillRunningProcess(out string processName, out int exitCode, out string error)
        {
            ...
            lock (this.processLock)
            {
                ...
                if (process != null)
                {
                    ...
                    return GSDPlatform.Instance.TryKillProcessTree(process.Id, out exitCode, out error);
                }
        ...
        {
            string packFilePath = Path.Combine(gitObjectsDirectory, GSDConstants.DotGit.Objects.Pack.Name, filenamePrefix);
