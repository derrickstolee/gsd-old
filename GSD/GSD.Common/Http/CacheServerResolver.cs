...
namespace GSD.Common.Http
{
    public class CacheServerResolver
    {
        ...
        public static string GetUrlFromConfig(Enlistment enlistment)
        {
            ...
            return
                GetValueFromConfig(git, GSDConstants.GitConfig.CacheServer, localOnly: true)
            ...
            out string error)
        {
            ...
            {
                cacheServer =
                    serverGSDConfig.CacheServers.FirstOrDefault(cache => cache.GlobalDefault)
                    ?? this.CreateNone();
            }
            else
            {
                cacheServer = serverGSDConfig.CacheServers.FirstOrDefault(cache =>
            ...
            ServerGSDConfig serverGSDConfig)
        {
            ...
            return
                serverGSDConfig.CacheServers.FirstOrDefault(cache => cache.Url.Equals(cacheServerUrl, StringComparison.OrdinalIgnoreCase))
        ...
        public bool TrySaveUrlToLocalConfig(CacheServerInfo cache, out string error)
        {
            ...
            GitProcess.Result result = git.SetInLocalConfig(GSDConstants.GitConfig.CacheServer, cache.Url, replaceAll: true);
        ...
        private static string GetDeprecatedCacheConfigSettingName(Enlistment enlistment)
        {
            ...
            return GSDConstants.GitConfig.GSDPrefix + sectionUrl + GSDConstants.GitConfig.DeprecatedCacheEndpointSuffix;
        }
