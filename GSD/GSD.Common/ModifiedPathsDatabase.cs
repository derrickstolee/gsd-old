...
namespace GSD.Common
{
    ...
    public class ModifiedPathsDatabase : FileBasedCollection
    {
        ...
        public static bool TryLoadOrCreate(ITracer tracer, string dataDirectory, PhysicalFileSystem fileSystem, out ModifiedPathsDatabase output, out string error)
        {
            ...
            {
                bool isRetryable;
                temp.TryAdd(GSDConstants.SpecialGitFiles.GitAttributes, isFolder: false, isRetryable: out isRetryable);
            }
        ...
        {
            string[] pathParts = modifiedPath.Split(new char[] { GSDConstants.GitPathSeparator }, StringSplitOptions.RemoveEmptyEntries);
            string parentFolder = string.Empty;
            for (int i = 0; i < pathParts.Length - 1; i++)
            {
                parentFolder += pathParts[i] + GSDConstants.GitPathSeparatorString;
        ...
        private string NormalizeEntryString(string virtualPath, bool isFolder)
        {
            ...
            return virtualPath.Replace(Path.DirectorySeparatorChar, GSDConstants.GitPathSeparator).Trim(GSDConstants.GitPathSeparator) +
                (isFolder ? GSDConstants.GitPathSeparatorString : string.Empty);
        }
    }
}
