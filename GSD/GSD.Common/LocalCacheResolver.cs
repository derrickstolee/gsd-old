...
namespace GSD.Common
{
    public class LocalCacheResolver
    {
        ...
        private const string MappingVersionKey = "GSD_LocalCache_MappingVersion";
            ...
            out string errorMessage)
        {
            ...
            {
                throw new ArgumentNullException(nameof(serverGSDConfig));
            }
            ...
            try
            {
                ...
                using (FileBasedLock mappingLock = GSDPlatform.Instance.CreateFileBasedLock(
                    this.fileSystem,
                    tracer,
                    lockPath))
                {
                    ...
                    if (this.TryOpenMappingFile(tracer, localCacheRoot, out mappingFile, out errorMessage))
                    {
                        try
                        {
                            ...
                            else
                            {
                                ...
                                if (this.TryGetLocalCacheKeyFromRemoteCacheServers(tracer, serverGSDConfig, currentCacheServer, mappingFile, out localCacheKey, out getLocalCacheKeyError))
                                {
                                    metadata.Add("localCacheKey", localCacheKey);
            ...
            out string error)
        {
            ...
            try
            {
                if (this.TryFindExistingLocalCacheKey(mappingFile, serverGSDConfig.CacheServers, out localCacheKey))
                {
                    EventMetadata metadata = CreateEventMetadata();
                    metadata.Add("currentCacheServer", currentCacheServer.ToString());
                    metadata.Add("localCacheKey", localCacheKey);
                ...
                foreach (CacheServerInfo cacheServer in serverGSDConfig.CacheServers)
                {
