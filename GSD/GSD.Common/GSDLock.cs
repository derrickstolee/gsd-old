...
namespace GSD.Common
{
    public partial class GSDLock
    {
        ...
        public void ReleaseLockHeldByGSD()
        {
            lock (this.acquisitionLock)
            {
                if (!this.currentLockHolder.IsGSD)
                {
                    throw new InvalidOperationException("Cannot release lock that is not held by GSD");
                }

                this.tracer.RelatedEvent(EventLevel.Verbose, nameof(this.ReleaseLockHeldByGSD), new EventMetadata());
        ...
        private bool ReleaseExternalLock(int pid, string eventName)
        {
            lock (this.acquisitionLock)
            {
                ...
                try
                {
                    if (this.currentLockHolder.IsGSD)
                    {
                        metadata.Add("IsLockedByGSD", "true");
                        return false;
                    }
        ...
        /// <summary>
        /// This class manages the state of which process currently owns the GSD lock. This code is complicated because
        ...
        ///
        /// The goal of this class is to make it impossible for the rest of GSDLock to read the external holder without being
        ...
        private class LockHolder
        {
            ...
            public bool IsFree
            {
                get { return !this.IsGSD && this.externalLockHolder == null; }
            }
            ...
            public void AcquireForGSD()
            {
                if (this.externalLockHolder != null)
                {
                    throw new InvalidOperationException("Cannot acquire for GSD because there is an external holder");
                }
            ...
            public NamedPipeMessages.LockData GetExternalHolder(out bool externalHolderTerminatedWithoutReleasingLock)
            {
                ...
                if (this.externalLockHolder != null)
                {
                    ...
                    externalHolderTerminatedWithoutReleasingLock = !GSDPlatform.Instance.IsProcessActive(pid);
                }
