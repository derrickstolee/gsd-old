...
namespace GSD.Common.Prefetch.Pipeline
{
        ...
        protected override void DoWork()
        {
            using (ITracer activity = this.tracer.StartActivity("ReadFiles", EventLevel.Informational))
            {
                ...
                while (this.availableBlobs.TryTake(out blobId, Timeout.Infinite))
                {
                    ...
                    {
                        bool succeeded = GSDPlatform.Instance.FileSystem.HydrateFile(Path.Combine(this.workingDirectoryRoot, modeAndPath.Path), buffer);
                        if (succeeded)
                        {
