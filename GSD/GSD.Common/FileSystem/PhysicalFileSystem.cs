...
namespace GSD.Common.FileSystem
{
    public class PhysicalFileSystem
    {
        ...
        {
            GSDPlatform.Instance.FileSystem.MoveAndOverwriteFile(sourceFileName, destinationFilename);
        }

        public virtual bool TryGetNormalizedPath(string path, out string normalizedPath, out string errorMessage)
        {
            return GSDPlatform.Instance.FileSystem.TryGetNormalizedPath(path, out normalizedPath, out errorMessage);
        }
        ...
        {
            return GSDPlatform.Instance.FileSystem.TryCreateDirectoryWithAdminAndUserModifyPermissions(directoryPath, out error);
        }
        ...
        {
            return GSDPlatform.Instance.FileSystem.TryCreateOrUpdateDirectoryToAdminModifyPermissions(tracer, directoryPath, out error);
        }
        ...
        public bool TryCopyToTempFileAndRename(string sourcePath, string destinationPath, out Exception handledException)
        {
            ...
            try
            {
                ...
                GSDPlatform.Instance.FileSystem.FlushFileBuffers(tempFilePath);
                this.MoveAndOverwriteFile(tempFilePath, destinationPath);
                return true;
            }
            catch (Win32Exception e)
