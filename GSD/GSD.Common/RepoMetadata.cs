...
namespace GSD.Common
{
    public class RepoMetadata
    {
        ...
        public static bool TryInitialize(ITracer tracer, string dotGSDPath, out string error)
        {
            return TryInitialize(tracer, new PhysicalFileSystem(), dotGSDPath, out error);
        }
        ...
        public bool TryGetOnDiskLayoutVersion(out int majorVersion, out int minorVersion, out string error)
        {
            ...
            try
            {
                ...
                {
                    error = "Enlistment disk layout version not found, check if a breaking change has been made to GSD since cloning this enlistment.";
                    return false;
                }
