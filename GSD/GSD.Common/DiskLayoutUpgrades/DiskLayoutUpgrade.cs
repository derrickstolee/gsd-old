...
namespace GSD.DiskLayoutUpgrades
{
    public abstract class DiskLayoutUpgrade
    {
        ...
        public static bool TryRunAllUpgrades(string enlistmentRoot)
        {
            ...
            foreach (DiskLayoutUpgrade upgrade in GSDPlatform.Instance.DiskLayoutUpgrade.Upgrades)
            {
                RegisterUpgrade(upgrade);
            }

            using (JsonTracer tracer = new JsonTracer(GSDConstants.GSDEtwProviderName, "DiskLayoutUpgrade"))
            {
                try
                {
        ...
        public static bool TryCheckDiskLayoutVersion(ITracer tracer, string enlistmentRoot, out string error)
        {
            ...
            try
            {
                ...
                {
                    if (majorVersion < GSDPlatform.Instance.DiskLayoutUpgrade.Version.MinimumSupportedMajorVersion)
                    {
                        error = string.Format(
                            "Breaking change to GSD disk layout has been made since cloning. \r\nEnlistment disk layout version: {0} \r\nGSD disk layout version: {1} \r\nMinimum supported version: {2}",
                            majorVersion,
                            GSDPlatform.Instance.DiskLayoutUpgrade.Version.CurrentMajorVersion,
                            GSDPlatform.Instance.DiskLayoutUpgrade.Version.MinimumSupportedMajorVersion);

                        return false;
                    }
                    else if (majorVersion > GSDPlatform.Instance.DiskLayoutUpgrade.Version.CurrentMajorVersion)
                    {
                        error = string.Format(
                            "Changes to GSD disk layout do not allow mounting after downgrade. Try mounting again using a more recent version of GSD. \r\nEnlistment disk layout version: {0} \r\nGSD disk layout version: {1}",
                            majorVersion,
                            GSDPlatform.Instance.DiskLayoutUpgrade.Version.CurrentMajorVersion);
                    ...
                    {
                        error = string.Format(
                            "GSD disk layout version doesn't match current version. Try running 'gvfs mount' to upgrade. \r\nEnlistment disk layout version: {0}.{1} \r\nGSD disk layout version: {2}.{3}",
                            majorVersion,
                            minorVersion,
                            GSDPlatform.Instance.DiskLayoutUpgrade.Version.CurrentMajorVersion,
                            GSDPlatform.Instance.DiskLayoutUpgrade.Version.CurrentMinorVersion);
            ...
            error = "Failed to read disk layout version. " + ConsoleHelper.GetGSDLogMessage(enlistmentRoot);
            return false;
        }
            ...
            out string error)
        {
            ...
            string dotGSDPath = Path.Combine(enlistmentRoot, GSDPlatform.Instance.Constants.DotGSDRoot);

            if (!GSDPlatform.Instance.DiskLayoutUpgrade.TryParseLegacyDiskLayoutVersion(dotGSDPath, out majorVersion))
            {
                if (!RepoMetadata.TryInitialize(tracer, dotGSDPath, out error))
                {
                    majorVersion = 0;
                    return false;
                }
