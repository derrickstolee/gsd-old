...
namespace GSD.UnitTests.Mock.Upgrader
{
    public class MockGitHubUpgrader : GitHubUpgrader
    {
        private string expectedGSDAssetName;
        ...
        public void PretendNewReleaseAvailableAtRemote(string upgradeVersion, GitHubUpgraderConfig.RingType remoteRing)
        {
            ...
            gitAsset.Name = "Git-2.17.1.gvfs.2.1.4.g4385455-64-bit" + GSDPlatform.Instance.Constants.InstallerExtension;
            gitAsset.Size = random.Next(int.MaxValue / 10, int.MaxValue / 2);
            gitAsset.DownloadURL = new Uri(assetDownloadURLPrefix + "/Git-2.17.1.gvfs.2.1.4.g4385455-64-bit" + GSDPlatform.Instance.Constants.InstallerExtension);
            release.Assets.Add(gitAsset);

            this.expectedGSDAssetName = gvfsAsset.Name;
        ...
        public override bool TrySetupUpgradeApplicationDirectory(out string upgradeApplicationPath, out string error)
        {
            ...
            upgradeApplicationPath = @"mock:\ProgramData\GSD\GSD.Upgrade\Tools\GSD.Upgrader.exe";
            error = null;
            return true;
        }
        ...
        protected override bool TryDownloadAsset(Asset asset, out string errorMessage)
        {
            ...
            if (this.expectedGSDAssetName.Equals(asset.Name, StringComparison.OrdinalIgnoreCase))
            {
                if (this.failActionTypes.HasFlag(ActionType.GSDDownload))
                {
                    errorMessage = "Error downloading GSD from GitHub";
                    return false;
                }
            }
            ...
            if (validAsset)
            {
                string fakeDownloadDirectory = @"mock:\ProgramData\GSD\GSD.Upgrade\Downloads";
        ...
        protected override bool TryDeleteDownloadedAsset(Asset asset, out Exception exception)
        {
            if (this.expectedGSDAssetName.Equals(asset.Name, StringComparison.OrdinalIgnoreCase))
            {
                if (this.failActionTypes.HasFlag(ActionType.GSDCleanup))
                {
                    exception = new Exception("Error deleting downloaded GSD installer.");
                    return false;
                }
        ...
        protected override void RunInstaller(string path, string args, string certCN, string issuerCN, out int exitCode, out string error)
        {
            ...
            if (fileName.Equals(this.expectedGitAssetName, StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    exitCode = -1;
                    error = "The contents of file C:\\ProgramData\\GSD\\GSD.Upgrade\\Tools\\Git-2.17.1.gvfs.2.1.4.g4385455-64-bit might have been changed by an unauthorized user or process, because the hash of the file does not match the hash stored in the digital signature. The script cannot run on the specified system. For more information, run Get-Help about_Signing.";
                }

                return;
            }

            if (fileName.Equals(this.expectedGSDAssetName, StringComparison.OrdinalIgnoreCase))
            {
                this.InstallerArgs.Add("GSD", installationInfo);
                this.InstallerExeLaunched = true;
                if (this.failActionTypes.HasFlag(ActionType.GSDInstall))
                {
                    exitCode = -1;
                    error = "GSD installation failed";
                }

                if (this.failActionTypes.HasFlag(ActionType.GSDAuthenticodeCheck))
                {
                    exitCode = -1;
                    error = "The contents of file C:\\ProgramData\\GSD\\GSD.Upgrade\\Tools\\SetupGSD.1.0.18297.1.exe might have been changed by an unauthorized user or process, because the hash of the file does not match the hash stored in the digital signature. The script cannot run on the specified system. For more information, run Get-Help about_Signing.";
                }
