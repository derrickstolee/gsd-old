...
namespace GSD.Service
{
    public class GSDMountProcess : IRepoMounter
    {
        ...
        public GSDMountProcess(ITracer tracer, MountLauncher processLauncher = null)
        {
            this.tracer = tracer;
        ...
        public bool MountRepository(string repoRoot, int sessionId)
        {
            string arguments = string.Format(
                "asuser {0} {1} mount {2}",
                sessionId,
                Path.Combine(GSDPlatform.Instance.Constants.GSDBinDirectoryPath, GSDPlatform.Instance.Constants.GSDExecutableName),
                repoRoot);

            if (!this.processLauncher.LaunchProcess(ExecutablePath, arguments, repoRoot))
            {
                this.tracer.RelatedError($"{nameof(this.MountRepository)}: Unable to start the GSD process.");
                return false;
            }
        ...
        public class MountLauncher
        {
            ...
            public virtual bool LaunchProcess(string executablePath, string arguments, string workingDirectory)
            {
                ...
                if (result.ExitCode != 0)
                {
                    ...
                    metadata.Add("Area", nameof(GSDMountProcess));
