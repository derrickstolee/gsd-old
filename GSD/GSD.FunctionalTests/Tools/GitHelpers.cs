...
namespace GSD.FunctionalTests.Tools
{
    public static class GitHelpers
    {
        ...
        /// GSD.FunctionalTests.LockHolder program.
        /// </summary>
        private const string LockHolderCommandName = @"GSD.FunctionalTests.LockHolder";
        private const string LockHolderCommand = @"GSD.FunctionalTests.LockHolder.dll";
        ...
        private static string LockHolderCommandPath
        {
            get
            {
                ...
                {
                    // On Windows, FT is run from the Output directory of GSD.FunctionalTest project.
                    // LockHolder is a .netcore assembly and can be found inside netcoreapp2.1
                    // subdirectory of GSD.FunctionalTest Output directory.
                    return Path.Combine(
                        Settings.Default.CurrentDirectory,
                        "netcoreapp2.1",
                        LockHolderCommand);
        ...
        public static void CheckGitCommandAgainstGSDRepo(string virtualRepoRoot, string command, params string[] expectedLinesInResult)
        {
            ProcessResult result = InvokeGitAgainstGSDRepo(virtualRepoRoot, command);
            result.Errors.ShouldBeEmpty();
            foreach (string line in expectedLinesInResult)
            {
                result.Output.ShouldContain(line);
        ...
        public static ProcessResult InvokeGitAgainstGSDRepo(
            string gvfsRepoRoot,
            string command,
            ...
            if (!string.IsNullOrEmpty(errors) && (removeWaitingMessages || removeUpgradeMessages))
            {
                ...
                IEnumerable<string> filteredErrorLines = errorLines.Where(line =>
                {
                    if (string.IsNullOrWhiteSpace(line) ||
                        (removeUpgradeMessages && line.StartsWith("A new version of GSD is available.")) ||
