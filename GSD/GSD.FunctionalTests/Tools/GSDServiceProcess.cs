...
{
    public static class GSDServiceProcess
    {
        ...
        public static string TestServiceName
        {
            get
            {
                ...
                {
                    // Service name is used in the lookup of GSD.Service communication pipes
                    // by clients for IPC. Custom pipe names can be specified as command line
                    // args to the Service during its startup. On the Mac, GSD.Service started
                    ...
                    // args (that are specific to testing) to GSD.Service (Service on Mac is a
                    // Launch agent and needs its args to be specified in its launchd.plist
                    // file). So on Mac - during tests GSD.Service is started without any
                    // customized pipe name for testing.
                    return "GSD.Service";
                }

                return "Test.GSD.Service";
            }
        }
        ...
        public static void UninstallService()
        {
            ...
            string serviceData = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData), "GSD", TestServiceName);
        ...
        private static string GetPathToService()
        {
            ...
            {
                ProcessResult result = ProcessHelper.Run("where", Properties.Settings.Default.PathToGSDService);
                result.ExitCode.ShouldEqual(0, $"{nameof(GetPathToService)}: where returned {result.ExitCode} when looking for {Properties.Settings.Default.PathToGSDService}");
                ...
                firstPath.ShouldNotBeNull($"{nameof(GetPathToService)}: Failed to find {Properties.Settings.Default.PathToGSDService}");
                return firstPath;
            }
            else
            {
                return Path.Combine(
                    ...
                    Properties.Settings.Default.PathToGSDService);
            }
        }
    }
}
