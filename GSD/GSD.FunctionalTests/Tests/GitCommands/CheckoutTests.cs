...
namespace GSD.FunctionalTests.Tests.GitCommands
{
    ...
    public class CheckoutTests : GitRepoTests
    {
        ...
        public void CheckoutBranchWithOpenHandleBlockingRepoMetdataUpdate()
        {
            ...
            Task task = Task.Run(() =>
            {
                ...
                while (attempts < 100)
                {
                    try
                    {
                        using (FileStream stream = new FileStream(Path.Combine(this.Enlistment.DotGSDRoot, "databases", "RepoMetadata.dat"), FileMode.Open, FileAccess.Read, FileShare.None))
                        {
                            fileLocked.Set();
                            testReady.Set();
                            Thread.Sleep(15000);
        ...
        public void CheckoutBranchWithStaleRepoMetadataTmpFileOnDisk()
        {
            ...
            this.FileSystem.WriteAllText(Path.Combine(this.Enlistment.DotGSDRoot, "databases", "RepoMetadata.dat.tmp"), "Stale RepoMetadata.dat.tmp file");
            this.ValidateGitCommand("checkout " + GitRepoTests.ConflictTargetBranch);
        }
        ...
        public void CheckoutBranchWhileOutsideToolDoesNotAllowDeleteOfOpenRepoMetadata()
        {
            ...
            Task task = Task.Run(() =>
            {
                ...
                while (attempts < 100)
                {
                    try
                    {
                        using (FileStream stream = new FileStream(Path.Combine(this.Enlistment.DotGSDRoot, "databases", "RepoMetadata.dat"), FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
                        {
                            fileLocked.Set();
                            testReady.Set();
                            Thread.Sleep(15000);
        ...
        public void CheckoutBranchWhileOutsideToolHasExclusiveReadHandleOnDatabasesFolder()
        {
            ...
            {
                int attempts = 0;
                string databasesPath = Path.Combine(this.Enlistment.DotGSDRoot, "databases");
                while (attempts < 100)
                {
