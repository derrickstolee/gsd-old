...
namespace GSD.FunctionalTests.Tests.EnlistmentPerTestCase
{
    ...
    public class PersistedWorkingDirectoryTests : TestsWithEnlistmentPerTestCase
    {
        ...
        {
            string enumerateDirectoryName = Path.Combine("GSD", "GSD");
            ...
            string[] subFiles = new string[]
            {
                ...
                Path.Combine(enumerateDirectoryName, "GSD.csproj"),
        ...
        public void PersistedDirectoryTests(FileSystemRunner fileSystem)
        {
            ...
            // Delete Folder Setup
            string deleteFolderName = Path.Combine("GSD", "GSD");
            ...
            // Write a file to GSD to ensure it has a physical folder
            string childFileToAdd = Path.Combine("GSD", "ChildFileToAdd.txt");
            string childFileToAddContent = "This is new child file in the GSD folder.";
            ...
            string childFolder = Path.Combine("GSD", "GSD.FunctionalTests");
            ...
            fileSystem.WriteAllText(postMountChildFilePath, postMountChildFileContent); // Verify we can create files in subfolders of GSD
            postMountChildFilePath.ShouldBeAFile(fileSystem).WithContents().ShouldEqual(postMountChildFileContent);

            // 663045 - Ensure that folder can be deleted after a new file is added and GSD is remounted
