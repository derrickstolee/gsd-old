...
NSString * const AutomountTitle = @"GSD AutoMount";
NSString * const AutomountStartMessageFormat = @"Attempting to mount %lu GSD repos(s)";
NSString * const AutomountSuccessMessageFormat = @"The following GSD repo is now mounted: \n%@";
NSString * const AutomountFailureMessageFormat = @"The following GSD repo failed to mount: \n%@";

@interface GSDNotification()
...
@implementation GSDNotification

+ (BOOL)tryValidateMessage:(NSDictionary *)jsonMessage
         buildNotification:(GSDNotification **)notification
                     error:(NSError *__autoreleasing *)error
{
    ...
    if (![identifier isKindOfClass:[NSNumber class]])
    {
        if (error != nil)
        {
            *error = [NSError errorWithDomain:GSDNotificationErrorDomain
                                         code:GSDInvalidMessageIdFormatError
    ...
    switch (idValue)
    {
        case AutomountStart:
        {
            *notification = [[GSDNotification alloc]
        ...
        {
            *notification = nil;
            initError = [NSError errorWithDomain:GSDNotificationErrorDomain
                                            code:GSDUnsupportedMessageError
                                 ...
                                 error:(NSError *__autoreleasing *)error
{
    ...
    if (self = [super init])
    {
        ...
        {
            *error = [NSError errorWithDomain:GSDNotificationErrorDomain
                                         code:GSDMissingEntitlementInfoError
