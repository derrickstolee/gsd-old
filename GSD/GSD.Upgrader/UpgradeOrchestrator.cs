...
namespace GSD.Upgrader
{
    public abstract class UpgradeOrchestrator
    {
        ...
        public void Execute()
        {
            ...
            {
                this.preRunChecker = new InstallerPreRunChecker(this.tracer, GSDConstants.UpgradeVerbMessages.GSDUpgradeConfirm);
            }
        ...
        protected bool LaunchInsideSpinner(Func<bool> method, string message)
        {
            return ConsoleHelper.ShowStatusWhileRunning(
                ...
                message,
                this.output,
                this.output == Console.Out && !GSDPlatform.Instance.IsConsoleOutputRedirectedToFile(),
                null);
        }
        ...
        private bool TryInitialize(out string errorMessage)
        {
            if (this.upgrader == null)
            {
                ...
                ProductUpgrader upgrader;
                if (!ProductUpgrader.TryCreateUpgrader(this.tracer, this.fileSystem, new LocalGSDConfig(), credentialStore, this.DryRun, this.NoVerify, out upgrader, out errorMessage))
                {
                    return false;
                }
        ...
        private bool TryRunUpgrade(out Version newVersion, out string consoleError)
        {
            ...
            if (!this.LaunchInsideSpinner(
                () =>
                {
                    ...
                    if (!this.TryCheckIfUpgradeAvailable(out newGSDVersion, out error))
                    {
                        return false;
                    }
                    ...
                    if (newGSDVersion != null && !this.TryDownloadUpgrade(newGSDVersion, out error))
                    {
                        return false;
                    }
            ...
            if (!this.LaunchInsideSpinner(
                () =>
                {
                    if (!this.preRunChecker.TryUnmountAllGSDRepos(out error))
                    {
                        return false;
                    }
        ...
        {
            EventMetadata metadata = new EventMetadata();
            string installedGSDVersion = ProcessHelper.GetCurrentProcessVersion();
            metadata.Add(nameof(installedGSDVersion), installedGSDVersion);

            GitVersion installedGitVersion = null;
            string error = null;
            string gitPath = GSDPlatform.Instance.GitInstallation.GetInstalledGitBinPath();
