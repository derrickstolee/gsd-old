...
namespace GSD.Service.UI
{
    public class GSDServiceUI
    {
        private const string ServiceAppId = "GSD";

        private readonly ITracer tracer;

        public GSDServiceUI(ITracer tracer)
        {
            this.tracer = tracer;
        }
        ...
        private void Start(string[] args)
        {
            ...
            using (NamedPipeServer server = NamedPipeServer.StartNewServer(GSDConstants.Service.UIName, this.tracer, this.HandleRequest))
            {
                ServiceController controller = new ServiceController(GSDConstants.Service.ServiceName);
                try
                {
                ...
                this.tracer.RelatedInfo("{0} stop detected -- exiting UI.", GSDConstants.Service.ServiceName);
            }
        }
