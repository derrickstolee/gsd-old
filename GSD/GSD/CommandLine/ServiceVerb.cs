...
{
    [Verb(ServiceVerbName, HelpText = "Runs commands for the GSD service.")]
    public class ServiceVerb : GSDVerb.ForNoEnlistment
    {
        ...
        public override void Execute()
        {
            ...
            {
                // Always ask the service to ensure that PrjFlt is enabled.  This will ensure that the GSD installer properly waits for
                // GSD.Service to finish enabling PrjFlt's AutoLogger
                string error;
        ...
        private bool TryGetRepoList(out List<string> repoList, out string errorMessage)
        {
            ...
            using (NamedPipeClient client = new NamedPipeClient(this.ServicePipeName))
            {
                if (!client.Connect())
                {
                    errorMessage = "GSD.Service is not responding.";
                    return false;
                }
