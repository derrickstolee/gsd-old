...
{
    public abstract partial class POSIXPlatform : GSDPlatform
    {
        ...
        protected POSIXPlatform() : this(
            underConstruction: new UnderConstructionFlags(
                supportsGSDUpgrade: false,
                supportsGSDConfig: false,
        ...
        public override bool TryGetGSDHooksPathAndVersion(out string hooksPaths, out string hooksVersion, out string error)
        {
            ...
            string binPath = Path.Combine(this.Constants.GSDBinDirectoryPath, GSDPlatform.Instance.Constants.GSDHooksExecutableName);
            if (File.Exists(binPath))
            {
                hooksPaths = binPath;
            }

            // TODO(POSIX): Get the hooks version rather than the GSD version (and share that code with the Windows platform)
        ...
        public override string GetGSDServiceNamedPipeName(string serviceName)
        {
            ...
            return this.GetDataRootForGSDComponent(serviceName) + ".pipe";
        }
        ...
        public override bool TryGetDefaultLocalCacheRoot(string enlistmentRoot, out string localCacheRoot, out string localCacheRootError)
        {
            ...
            try
            {
                localCacheRoot = Path.Combine(homeDirectory, GSDConstants.DefaultGSDCacheFolderName);
        ...
        public abstract class POSIXPlatformConstants : GSDPlatformConstants
        {
            ...
            {
                get { return new HashSet<string>(StringComparer.OrdinalIgnoreCase) { "GSD.Mount", "git", "wish" }; }
            }
