...
PATH_STRING GetGSDPipeName(const char *appName)
{
    // The pipe name is built using the path of the GSD enlistment root.
    // Start in the current directory and walk up the directory tree
    // until we find a folder that contains the ".gvfs" folder

    const size_t dotGSDRelativePathLength = sizeof(L"\\.gvfs") / sizeof(wchar_t);

    // TODO 640838: Support paths longer than MAX_PATH
    wchar_t enlistmentRoot[MAX_PATH];
    DWORD currentDirResult = GetCurrentDirectoryW(MAX_PATH - dotGSDRelativePathLength, enlistmentRoot);
    if (currentDirResult == 0 || currentDirResult > MAX_PATH - dotGSDRelativePathLength)
    {
    ...
    while (1)
    {
        ...
        dotGSDHandle = FindFirstFileW(enlistmentRoot, &findFileData);
        if (dotGSDHandle != INVALID_HANDLE_VALUE)
        {
            FindClose(dotGSDHandle);
        ...
        {
            die(ReturnCode::NotInGSDEnlistment, "%s must be run from inside a GSD enlistment\n", appName);
        }
    ...
    *(lastslash) = 0;
    ...
    return L"\\\\.\\pipe\\GSD_" + namedPipe;
}

PIPE_HANDLE CreatePipeToGSD(const PATH_STRING& pipeName)
{
