...
PATH_STRING GetGSDPipeName(const char *appName)
{
    ...
    while (1)
    {
        *lastslash = '\0';
        ...
        {
            die(ReturnCode::NotInGSDEnlistment, "Failed to open directory: %s, error: %d\n", enlistmentRoot, errno);
        }
        ...
        if (errno != 0)
        {
            die(ReturnCode::NotInGSDEnlistment, "readdir failed in directory: %s, error: %i\n", enlistmentRoot, errno);
        }
    ...
    *(lastslash) = 0;
    ...
    return PATH_STRING(enlistmentRoot) + "/.gvfs/GSD_NetCorePipe";
}
