// GSD.ReadObjectHook
//
// When GSD installs GSD.ReadObjectHook, it copies the file to
...
// GSD.ReadObjectHook decides which GSD instance to connect to based on its path.
// It then connects to GSD and asks GSD to download the requested object (to the .git\objects folder).
...
int main(int, char *argv[])
{
    ...
    PATH_STRING pipeName(GetGSDPipeName(argv[0]));

    PIPE_HANDLE pipeHandle = CreatePipeToGSD(pipeName);
